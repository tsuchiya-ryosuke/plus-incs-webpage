...
/*
 * 体験ゴール定義（3軸）
 * - 信頼: 画面を跨いでも同じ構造・同じ語彙で理解できる安心感を維持する。
 *   UIで見せる: ヘッダー/タブ/状態ラベルの配置とスタイルを固定して情報階層を崩さない。
 * - 前向きさ: 入力・送信・完了の流れで、利用者が前に進めた実感を得られること。
 *   UIで見せる: 成功画面と主要CTAをポジティブな文言と明確な主従関係で提示する。
 * - 進捗実感: 提出可否・確認済み項目・次にやる行動を即座に把握できること。
 *   UIで見せる: 提出済/未提出や確認項目を一覧比較しやすい形で常に可視化する。
 *
 * 既存コンポーネント改修優先度
 * - P0: Header（信頼の基盤。画面遷移時の現在地・文言・階層を統一）
 * - P0: InputC（前向きさの要。入力から送信までの行動導線を最優先で改善）
 * - P1: VerificationChecklist（進捗実感。検証項目を3軸へ再マッピング）
 * - P2: Nav（visual側との整合対象。Web/モバイル共通ルールとして追従）
